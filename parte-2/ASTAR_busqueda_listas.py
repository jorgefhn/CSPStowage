...
class Node:
	...
	def expandir(self):
		...
		if self.checkNavigate():
			...
			new_node = self.generateNode(self.state.contenedores,self.g+3500,self.state.asignados,self.state.mapa,self.state.puerto_del_barco+1) #generamos nuevo nodo
		...
		for cont in range(len(self.state.contenedores)):
			...
			if action == "descarga":  # hay que descargar
				...
				x_nuevo,y_nuevo = new_node.state.contenedores[cont_nuevo][0], new_node.state.contenedores[cont_nuevo][1]
	...
	def checkAction(self, cont: int):
		...
		if (puerto_contenedor != "B" and puerto_destino > puerto_contenedor and puerto_destino > self.state.puerto_del_barco): # !=
			return "carga"
	...
	def checkNavigate(self):
		...
		for c in range(len(self.state.contenedores)): #recorre todos los contenedores y comprueba si están cargados/descargados
	...
	def checkReordenate(self,posicion:int,x:int,y:int):
		...
		for fila in range(0,x): #itera sobre toda la pila
			...
			if self.state.mapa[fila][y] not in ("N","E","X"): #es una posición asignada
				...
				pos_comparar = self.state.contenedores.index([fila,y,"B"]) #buscamos la posición en el array de cotnenedores
	...
	def cargar(self, posicion: int,x:int,y:int):
		...
		if self.state.contenedores[posicion][2] != "B" and self.comprobar_no_asignado(x,y) and self.contenedor_compatible(posicion,x,y):
	...
	def comprobar_asignado(self, x, y):
		...
		return ( (x, y) in valores and mapa[x][y] != "X" and ( (x < max_prof - 1 and (mapa[x + 1][y] == "X" or (x+1,y) in valores) ))  or (x == max_prof - 1) ) #esta comprobación no funciona
	...
	def contenedor_compatible(self,posicion,x,y):
		...
		if (contenedor[1] == "R" and self.state.mapa[x][y] == "E") or (contenedor[1] == "S" and self.state.mapa[x][y] in ("E","N")):
...
def busqueda(nodo_inicial,nodo_final):
	...
	while (len(abierta)> 0) and not exito:
		...
		minimo = abierta.pop(0)

		if minimo not in cerrada:

			print("Minimo:", minimo)
			print("Nodo final: ",nodo_final)
			print(minimo.state == nodo_final.state)
...
busqueda(nodo_inicial,nodo_final)
